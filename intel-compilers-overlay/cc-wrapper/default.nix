  ...
  installPhase =
    ...
    '') + ''
      ...
      if [ -e $ccPath/${prefix}gcc ]; then
        ...
        wrap ${prefix}icc ${./cc-wrapper.sh} $ccPath/icc
        ln -s ${prefix}icc $out/bin/${prefix}cc
        export named_cc=${prefix}icc
        export named_cxx=${prefix}icpc
      fi
      ...
      if [ -e $ccPath/${prefix}g++ ]; then
        ...
        wrap ${prefix}icpc ${./cc-wrapper.sh} $ccPath/icpc
        ln -s ${prefix}icpc $out/bin/${prefix}c++
      fi
    ...
    + optionalString cc.langFortran or false ''
      export named_f90=${prefix}f90
      if [ -e $ccPath/${prefix}gfortran ]; then
        wrap ${prefix}gfortran ${./cc-wrapper.sh} $ccPath/${prefix}gfortran
        ln -sv ${prefix}gfortran $out/bin/${prefix}f90
        ln -sv ${prefix}gfortran $out/bin/${prefix}g77
        ln -sv ${prefix}gfortran $out/bin/${prefix}f77
      elif [ -e $ccPath/ifort ]; then
        wrap ${prefix}ifort ${./cc-wrapper.sh} $ccPath/${prefix}ifort
        ln -sv ${prefix}ifort $out/bin/${prefix}f90
      fi
    ''
